...
impl <T> Proof<T> {
    ...
    fn validate_recursive(&self, path_item: &PathItem) -> bool {
        match path_item.sub_item {
            Some(ref child) => match child.sibling_hash {
...
impl PathItem {
    ...
    pub fn create_path<T: Hashable> (node: &TreeElement<T>, hash_to_find: &[u8]) -> Option<PathItem> {
        match *node {
            TreeElement::Node { ref left, ref right, ref hash } => PathItem::new_node_proof(hash, hash_to_find, left, right),
    ...
    fn new_node_proof<T: Hashable> (hash: &[u8], hash_to_find: &[u8], left: &TreeElement<T>, right: &TreeElement<T>) -> Option<PathItem> {
